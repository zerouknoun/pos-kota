...
<head>
    ...
    <title>Edit Menu - KOPI TAKALAR</title>
...
<body class="bg-gray-50 font-sans antialiased text-gray-900">
    ...
    <main class="max-w-xl mx-auto px-4 py-12">
        ...
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-800">Edit Menu Produk</h1>
            <p class="text-gray-500 text-sm mt-0.5">Perbarui rincian, kategori, atau harga dari menu produk terpilih.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            <form action="{{ route('products.update', $product->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required placeholder="Contoh: Kopi Susu Caramel" value="{{ old('name', $product->name) }}"
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                            class="w-full bg-gray-50 border border-gray-200 focus:border-[#54E868] focus:ring-1 focus:ring-[#54E868] rounded-xl py-3 px-4 text-sm text-gray-800 outline-none transition-all">
                        <option value="A. ICE" {{ old('category', $product->category) === 'A. ICE' ? 'selected' : '' }}>🥤 A. ICE</option>
                        <option value="B. HOT" {{ old('category', $product->category) === 'B. HOT' ? 'selected' : '' }}>☕ B. HOT</option>
                        <option value="C. Gelas" {{ old('category', $product->category) === 'C. Gelas' ? 'selected' : '' }}>🥛 C. Gelas</option>
                    </select>
                    @error('category')
                        <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="price" id="price" required placeholder="Contoh: 18000" min="0" value="{{ old('price', $product->price) }}"
